...
class TextEditor {
 public:
  ...
  class IAutocompleteProvider {
   public:
    ...
    virtual void RequestCompletions(const std::string& filePath, int line,
                                    int column, const std::string& prefix) = 0;

    // Called when text changes. The provider can use this for incremental sync.
    virtual void OnTextChanged(const std::string& filePath,
  ...
  // Render a single line of code with syntax highlighting (no newline handling).
  // Use for inline code in tooltips like signature help.
  void RenderCodeLine(const char* first, const char* last);

  // Convert a screen position to text coordinates (for mouse hover).
  // Uses the content origin stored during the last Render() call.
  // Must be called after Render() has run at least once.
  ImVec2 GetContentOrigin() const { return mContentOrigin; }

  Coordinates GetCoordinatesAt(const ImVec2& screen_pos) const {
    return ScreenPosToCoordinates(screen_pos, mContentOrigin);
  }

  // Render markdown-style text with code fence support.
  // Code blocks (```lang ... ```) are rendered with syntax highlighting
  // using the current language/palette and monospace font.
  // Everything else is rendered as plain text with the default font.
  // monoFont can be null if the caller already pushed the right font.
  void RenderMarkdown(const std::string& text, ImFont* mono_font = nullptr,
 ...
 private:
  ...
  ImVec2 mCharAdvance;
  ImVec2
      mContentOrigin;  // screen-space origin of text content, set during Render()
